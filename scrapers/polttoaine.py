...
# Table columns: [0] location, [1] date, [2] 95E10, [3] 98E, [4] diesel
FUELS = {
...
def fetch_prices(fuel: str = "95E10") -> list[dict]:
    """Returns up to 20 cheapest stations for the given fuel kind."""
...
if __name__ == "__main__":
    for kind in ("95E10", "diesel"):
        print(f"--- {kind} ---")
        for row in fetch_prices(kind):
            print(f"{row['price']:.3f} EUR  {row['city']:<15} {row['station']}")
